...
import { AuthService } from '@microservice/ui-common';

/**
 * SSO Callback Component for Kanban UI
 *
 ...
 * 4. Redirects to kanban board with clean URL
 */
@Component({
  ...
  template: `
    <div style="display: flex; align-items: center; justify-content: center; height: 100vh; flex-direction: column; gap: 1rem;">
      ...
      <div style="font-size: 0.9rem; color: #999;">Please wait while we securely log you in to Kanban</div>
    </div>
  `
})
export class SsoCallbackComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (window.opener) {
      window.opener.postMessage({ type: 'SSO_READY' }, window.location.origin.replace('4202', '4200'));
    }
  ...
  private handleMessage(event: MessageEvent) {
    ...
    if (event.data?.type === 'SSO_TOKEN' && event.data?.token) {
      ...
      // Redirect to kanban boards with clean URL (no token in URL)
      console.log('[SSO] Redirecting to kanban boards...');
